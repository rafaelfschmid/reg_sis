...
//#define interpol_linear( x0,  x1,  y0,  y1,  x) ((y1 - y0) * (x - x0) / (x1 - x0) + y0)
#define interpol_linear( x0,  y0,  y1,  x) ((y1 - y0) * (x - x0) + y0)
        ...
        float *stack, float dt, float idt, int tau, int w)
{
    ...
    float num[w], den[w];
    memset(&num[0], 0, sizeof(num)); // Setando para 0
    memset(&den[0], 0, sizeof(den)); // Setando para 0
    int M = 0, skip = 0;
    float _stack = 0;
